...
- Known for innovation, research, and transformative learning; located in Chennai.
- Taglin/Motto - "High on tech"
- How to register for events/registration link/payment portal - https://chennaievents.vit.ac.in/technovit/
- both internal and external partipants are allowed to participate and even form mixed teams.
- Title sponsor: HCL Tech, FM partner: Hello FM
- 150+ events, even international students from across 20+ countries come.
...
export async function POST(req: Request) {
  try {
    const { messages = [] } = await req.json(); // Destructure with a default empty array

    // The explicit Array.isArray check is still good practice if you want to
    // return a specific 400 error for non-array inputs, but defaulting to []
    // already prevents the 'map' error.
    // if (!Array.isArray(messages)) {
    //   console.error("Chat API received invalid messages format: expected an array but got:", messages);
    //   return new Response("Invalid message format received by API.", { status: 400 });
    // }
